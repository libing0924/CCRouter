...
@property(nonatomic, strong) NSMutableDictionary<NSNumber *, NSMutableArray<id<LBModuleProtocol>> *> *BHModulesByEvent;
@property(nonatomic, strong) NSMutableDictionary<NSNumber *, NSString *> *BHSelectorByEvent;

@end

@implementation LBModuleManager
...
+ (instancetype)sharedManager
{
    ...
    dispatch_once(&onceToken, ^{
        sharedManager = [[LBModuleManager alloc] init];
    });
...
- (void)loadLocalModules
{
    ...
    NSString *plistPath = [[NSBundle mainBundle] pathForResource:[LBModuleContext shareInstance].moduleConfigName ofType:@"plist"];
...
- (void)unRegisterDynamicModule:(Class)moduleClass {
    ...
    [self.BHModulesByEvent enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull key, NSMutableArray<id<LBModuleProtocol>> * _Nonnull obj, BOOL * _Nonnull stop) {
        __block NSInteger index = -1;
        [obj enumerateObjectsUsingBlock:^(id<LBModuleProtocol>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:moduleClass]) {
                index = idx;
                *stop = NO;
            }
...
- (void)registedAllModules
{
    ...
    [self.BHModuleInfos enumerateObjectsUsingBlock:^(NSDictionary *module, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (NSStringFromClass(moduleClass) && !hasInstantiated) {
            id<LBModuleProtocol> moduleInstance = [[moduleClass alloc] init];
     ...
     shouldTriggerInitEvent:(BOOL)shouldTriggerInitEvent
{
    ...
    if ([class conformsToProtocol:@protocol(LBModuleProtocol)]) {
        ...
        id<LBModuleProtocol> moduleInstance = [[class alloc] init];
        [self.BHModules addObject:moduleInstance];
        [moduleInfo setObject:@(YES) forKey:kModuleInfoHasInstantiatedKey];
        [self.BHModules sortUsingComparator:^NSComparisonResult(id<LBModuleProtocol> moduleInstance1, id<LBModuleProtocol> moduleInstance2) {
...
{
    [self.BHModules enumerateObjectsUsingBlock:^(id<LBModuleProtocol> moduleInstance, NSUInteger idx, BOOL * _Nonnull stop) {
        [self registerEventsByModuleInstance:moduleInstance];
    }];
}

- (void)registerEventsByModuleInstance:(id<LBModuleProtocol>)moduleInstance
{
...
- (NSMutableDictionary<NSNumber *, NSMutableArray<id<LBModuleProtocol>> *> *)BHModulesByEvent
{
...
- (void)handleModuleEvent:(NSInteger)eventType
                forTarget:(id<LBModuleProtocol>)target
          withCustomParam:(NSDictionary *)customParam
{
    switch (eventType) {
        case BHMInitEvent:
...
- (void)handleModulesInitEventForTarget:(id<LBModuleProtocol>)target
                        withCustomParam:(NSDictionary *)customParam
{
    LBModuleContext *context = [LBModuleContext shareInstance].copy;
...
- (void)handleModulesTearDownEventForTarget:(id<LBModuleProtocol>)target
                            withCustomParam:(NSDictionary *)customParam
{
    LBModuleContext *context = [LBModuleContext shareInstance].copy;
    context.customParam = customParam;
